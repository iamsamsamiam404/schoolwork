...
*** 5. Installed Windows Server 2019 Virtual Machine:
   ...
   - Selected an ESXi host (e.g., 44.100.10.91).
...
*** 8. Migrated Compute Resource (vMotion):
   ...
   - Clicked *Next*.
   - Selected the target ESXi host (e.g., `44.100.10.92`).
...
- Migration of Windows 10 VM
   + The Windows 10 VM was successfully migrated from VMware Workstation to the ESXi server and added to the inventory.
...
\centering
|                        | *ESXi1.2 Server* | *ESXi2.1 Server* | *vCenter*     | *Windows 10 VM* | *Windows 2019 Srv.* |
|------------------------+------------------+------------------+---------------+-----------------+---------------------|

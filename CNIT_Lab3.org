...
*Navigated to ESX: 2. I's IP address*
- Found the new server "SCDCOZ"
- Navigated through IVM Hardware| Edit | Network Adapter 1 |Browse | site2 VM network | ok | ok |
...
- [ ] Create a share on a Windows 10 client in your outer virtualization layer to use as a backup target
 + [ ] Add a new disk to the Windows 10 client VM to house backups
- [ ] To conserve disk space in the lab environment, backup only user files and the domain controller database (DCDB)
